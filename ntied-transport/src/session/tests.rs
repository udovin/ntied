...
fn rekey_grace_period() {
    ...
    let ct = resp.on_rekey_data(&epk).expect("expected ciphertext");
    assert!(init.on_rekey_ack_data(&ct));
...
fn on_auth_data_success() {
    ...
    assert!(session.on_auth_data(&public_key, &sig));
...
fn rekey_duplicate() {
    ...
    let ct1 = resp.on_rekey_data(&epk).expect("expected ciphertext");
    let ct2 = resp.on_rekey_data(&epk).expect("expected ciphertext");

    assert_eq!(ct1.to_bytes(), ct2.to_bytes(), "Duplicate Rekey should return identical RekeyAck");
...
fn rekey_simultaneous() {
    ...
    let ct = resp.on_rekey_data(&epk_init).expect("expected ciphertext");

    // Initiator receives Responder's Rekey - wins tie-breaker, ignores
    assert!(init.on_rekey_data(&epk_resp).is_none());
...
fn on_rekey_ack_data_without_start() {
    ...
    // Generate a valid ciphertext from a rekey, but give it to an initiator that never started rekey
    let epk = resp.start_rekey().unwrap();
    let ct = init.on_rekey_data(&epk).unwrap();
    // Fresh initiator (never called start_rekey) should reject
    let (keys_c, _, th2) = make_key_pair();
    let mut fresh = Session::new(Role::Initiator, 1, keys_c, th2);
    assert!(!fresh.on_rekey_ack_data(&ct));
}
...
fn start_rekey_idempotent() {
    ...
    assert_eq!(epk1.to_bytes(), epk2.to_bytes());
}
...
fn on_rekey_data_responder_different_payload() {
    ...
    assert_ne!(epk1.to_bytes(), epk2.to_bytes());
...
fn on_rekey_ack_data_wrong_epoch() {
    ...
    // Try processing the same ack again — transition is cleared, should return false
    assert!(!init.on_rekey_ack_data(&ct));
}
