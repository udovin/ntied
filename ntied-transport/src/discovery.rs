...
pub struct ConnectionRequest {
    ...
    // Can be None if we can not get connection_id of incoming connection request.
    // This can happen when Discovery implementation does not support sending connection_id notifications.
...
pub trait Discovery: Send + Sync {
    // Should send to network notification that we want to connect with peer with specified public_key and connection_id of connection.
    //
    ...
    // Also, optional connection_id and public_key of peer that we want to connect with that should be used for connection establishment.
