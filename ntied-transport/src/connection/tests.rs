...
fn make_test_pair() -> TestPair {
    ...
    let initiator = Connection::new(session_i, 100, 200, true, init_auth);
    let responder = Connection::new(session_r, 200, 100, false, resp_auth);
...
fn deliver(src: &mut Connection, dst: &mut Connection, now: Instant) -> usize {
